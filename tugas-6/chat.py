...
class Chat:
    ...
    def proses(self, data):
        ...
        try:
            ...
            if command == 'login':
                ...
                username_from = self.sessions[session_id]['username']
                logging.warning('Send to server {}: session {} sent message from {} to {}'.format(server_id, session_id, username_from, username_to))
                ...
                logging.warning("Send: session {} sent message from {} to {}".format(session_id, usernamefrom, group_id))
    ...
    def server_inbox(self, username_from, username_to, message):
        ...
        msg = {'msg_from': s_fr['name'], 'msg_to': s_to['name'], 'msg': message, 'sent_time': curr_time.strftime("%Y-%m-%d %H:%M:%S")}
    ...
    def send_message(self, session_id, username_from, username_to, message):
        ...
        curr_time = datetime.now()
        message = {'msg_from': s_fr['name'], 'msg_to': s_to['name'], 'msg': message, 'sent_time': curr_time.strftime("%Y-%m-%d %H:%M:%S")}
        outqueue_sender = s_fr['outgoing']
        inqueue_receiver = s_to['incoming']
        try:
    ...
    def send_to_server(self, session_id, server_id, username_to, message):
        ...
        username_from = self.sessions[session_id]['username']
        self.servers[server_id].put('serverinbox {} {} {}'.format(username_from, username_to, message))
    ...
    def send_group_message(self, session_id, username_from, group_id, server_from, message):
        ...
        for member in self.groups[group_id]:
            ...
            if server_to == server_from:
                self.send_message(session_id, username_from, username_to, message)
            else:
                self.send_to_server(session_id, server_to, username_to, message)
